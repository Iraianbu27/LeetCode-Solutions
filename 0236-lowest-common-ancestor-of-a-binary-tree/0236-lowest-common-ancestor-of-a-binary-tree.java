 ...
 */
 ...
 //if the root searching for is required then add the temp list to result List to have the track of the path
// class Solution {
...
//         List<TreeNode> list = new ArrayList<>(); //O(H)
//         List<List<TreeNode>> result = new ArrayList<>(); //O(2H)
//         node2Leaf(root,p,q,list,result); //O(N)
//         List<TreeNode> list1 = result.get(0); //O(H)
//         List<TreeNode> list2 = result.get(1);//O(H)
//         int len = Math.min(list1.size(),list2.size());
//         TreeNode lastNode = null;
//             for(int i = 0;i<len;i++){ //O(H)
